...
import { SymbolItemNavigation } from './better-navigation';
import { ContextKey } from './utils';

export class Navigation {

	private readonly _disposables: vscode.Disposable[] = [];
	private readonly _ctxCanNavigate = new ContextKey<boolean>('better-navigation.canNavigate');

	private _delegate?: SymbolItemNavigation<unknown>;

	constructor(private readonly _view: vscode.TreeView<unknown>) {
		this._disposables.push(
			vscode.commands.registerCommand('better-navigation.next', () => this.next(false)),
			vscode.commands.registerCommand('better-navigation.prev', () => this.previous(false)),
		);
	}
